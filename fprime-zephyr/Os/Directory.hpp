...
struct ZephyrDirectoryHandle : public DirectoryHandle {
    //! Maximum length of a stored directory path, including the null terminator
    static constexpr FwSizeType PATH_MAX_LENGTH = 256;

    DIR* m_dir_descriptor = nullptr;
    //! Path the directory was opened with. Zephyr declares but does not implement
    //! rewinddir(), so rewind() must close and reopen the directory by path.
    char m_path[PATH_MAX_LENGTH] = {};
};

//! \brief Zephyr implementation of Os::Directory
//!
//! Uses Zephyr's POSIX opendir/readdir/closedir shim. rewind() is emulated by
//! reopening the directory since Zephyr provides no rewinddir implementation.
